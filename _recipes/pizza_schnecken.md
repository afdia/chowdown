---
layout: recipe
title: "Pizza Schnecken"
image: pizza_schnecken.webp
category: Hauptgericht

images:
  - image: ../images/pizza_schnecken_vor_dem_rollen.webp
    description: Hier sieht man die Schnecken in allen Zubereitungsstadien. Nachdem ich sie gleichzeitig ausgerollt habe, sind sie beim einrollen schon recht stark auf der Silikonmatte geklebt
  - image: ../images/pizza_schnecken_einrollen.webp
    description: Mit dem Backpapier kann man die Rolle sehr gut einrollen (hier hat es noch weniger geklebt, erst bei der Silikonmatte war es dann ungut klebrig)
  - image: ../images/pizza_schnecken_vor_dem_backen.webp
    description: Eines der 2 Blech vor dem Backen
  - image: ../images/pizza_schnecken_nach_dem_backen.webp
    description: Dasselbe nach ca 18min im Rohr
  - image: ../images/pizza_schnecken_ausgelaufen.webp
    description: Hier sieht man die 3. Rolle wo der Käse stark ausgelaufen ist. Vmtl wars zuviel Käse (oder zuviel Mozzarella, Feuchtigkeit weils der Rest vom Käseteller war mit mehr Flüssigkeit als vorher). Geschmacklich schon gut aber eben nicht mehr schön. Die Pizzastangerl waren OK aber trocken, die muss man wie Pizza bei 300°C machen damit sie nicht so hart werden

ingredients:
  - Pizzateig (siehe Pizza Rezept)
  - Mutti Sauce
  - 150g Almkönig (vmtl +100g nötig weil wir beim 1. Versuch nur 3 Teigrollen gemacht haben und den 4. Teig zu Pizzastangerl verarbeitet haben)
  - 125g Mozzarella
  - beliebiger Belag

directions:
  - Pizzateig wie bei Pizza Rezept machen und 1h rasten lassen
  - Käse in feine Würfel schneiden und miteinander vermischen
  - In 4 Teile zerteilen, etwas flach machen und 10min rasten lassen
  - Dann jeden Teig breit ausrollen, mit Mutti Tomatensauce bestreichen und mit Käse bzw anderem Belag belegen
  - Von unten nach oben einrollen (geht mit Backpapier einfacher, aber ist auch auf Silikonmatte möglich). Achtung man sollte sich nicht zuviel Zeit lassen, je länger der Teig ausgerollt liegt desto mehr pickt und lässt sich schwerer einrollen
  - Eingerollten Teig mit Messer in ca 1cm breite Rollen schneiden (evtl Backpapier schon auf Backblech oder Brett legen damit man nicht auf der Arbeitsfläche schneidet). Alternativ mit Teigkarte Scheiben runterdrücken, geht schneller, wird aber nicht so schön - evtl zerfließt der Käse dann auch mehr als wenn man es schneidet
  - Schnecken auf 2 Backblech verteilen und für ca 15-18min ins 220°C Umluft vorgeheizte Backrohr geben. Nach 10 und 15min kurz die Tür öffnen und Dampf raulassen

---

Ergebnis war frisch aus dem Backrohr nicht so gut wie erhofft und Käse ist stark ausgeronnen.
Ausgekühlt waren sie geschmacklich dann viel besser! Daher gutes Gericht zum Vorbereiten.
Evtl keinen Mozzarella nehmen weil der ausrinnt? Oder probieren die Schnecken und Muffinformen zu geben, dann können sie nicht ausrinnen